...
import FriendRatingsRepository from "../infrastructure/repositories/friendRatings.repository";
...
export default class FriendRatingsUseCase implements IFriendRatingsUseCase {
  ...
  async  followUnfollow(userId: string, friendId: string): Promise<void> {
    ...
    async retriveFollowerFollowing(userId: string): Promise<{following:string[],followers:string[]}> {
        try {
            const following:string[] = await this.friendRatingsRepository.findAllfollowings(userId);
             const followers:string[] = await this.friendRatingsRepository.findAllfollowers(userId);
